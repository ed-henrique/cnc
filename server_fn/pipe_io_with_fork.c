...
void command_output_with_fork(char *command, char *output, int BUFFER_SIZE) {
  ...
  if (pipe(pipe_flag) == -1)
    error_output((char *)"Could Not Pipe");

  if ((pid = fork()) == -1)
    error_output((char *)"Could Not Fork");
  ...
  if (pid == 0) {
    ...
    error_output((char *)"Could Not Execute");
  }
  ...
  if (output[0] == '\0') {
    ...
    strcpy(
        output,
        "Output was either blank or command doesn't exist. Please, retry!\n");
  }
}