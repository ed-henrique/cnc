...
// This variable is atomic because there is a need for multiple threads to access it simultaneously
...
void *handle_client(void* client) {
	...
	if (recv(cli->sockfd, cli->name, LEN, 0) == -1) error_output("Could Not Receive Client's Name");
	if (recv(cli->sockfd, &cli->compress, sizeof(int), 0) == -1) error_output("Could Not Receive Compress Flag");
	...
	while (1) {
		...
		if (cli->compress) {
			...
			// The first one receives the command original size, before compression
			// The second one receives the compressed command size
			// The third one receives the compressed command
			...
			if (recv(cli->sockfd, &command_byte_size, sizeof(ulong), 0) == -1) error_output("Could Not Receive Compressed Command Size");

			// Allocates enough memory for decompressed command size
			char* command = (char*)malloc(command_byte_size * sizeof(char));

			if (recv(cli->sockfd, command, command_byte_size, 0) == -1) error_output("Could Not Receive Compressed Command");
			...
			// The first one sends the output original size, before compression
			// The second one sends the compressed output size
			// The third one sends the compressed output

			if (send(cli->sockfd, &output_size, sizeof(ulong), 0) == -1) error_output("Could Not Send Output Size");
			if (send(cli->sockfd, &output_byte_size, sizeof(ulong), 0) == -1) error_output("Could Not Send Compressed Output Size");
			...
			if (receive == -1) error_output("Could Not Receive Uncompressed Command");
			else if (receive == 0) {
		        // bzero() set bytes to 0, flushing this buffer
				bzero(output, BUFFER_SIZE);
				bzero(command, BUFFER_SIZE);
			...
			// Output is sent
			if (send(cli->sockfd, output, strlen(output), 0) == -1) error_output("Could Not Send Uncompressed Output");
...
int main(int argc, char* argv[]){
	...
	// File descriptors for listen and connect
	...
	// Set socket options
	if(setsockopt(listen_fd, SOL_SOCKET,(SO_REUSEPORT | SO_REUSEADDR), (char*)&option,sizeof(option)) < 0) error_output("Could Not Set Socket Options");

	// Bind server to IP
    if(bind(listen_fd, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0) error_output("Could Not Bind Server");

	// Listening for clients
    if (listen(listen_fd, 10) < 0) error_output("Could Not Listen");
	...
	while(1){
		...
		if (conn_fd == -1) error_output("Could Not Connect To Client");
