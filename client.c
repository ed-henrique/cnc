...
char HOST[16] = "127.0.0.1"; // Set as default, can be changed using --host
...
// Signal handler for when a SIGINT is received, activating flag to disconnect
// from server
void catch_sigint(int sig) { disconnect = 1; }
...
void send_msg() {
  ...
  while (1) {
    // This loop waits for the response to be displayed before prompting user
    // for input again
    if (!EMPTY_COMMAND)
    ...
    // Sends command
    if (send(socket_connection, user_command, strlen(user_command), 0) == -1)
      error_output((char *)"Could Not Send Command Without Compression");

    // Change log flag to send
    SEND_OR_RECEIVE = 0;
    if (LOG_SAVE)
      update_log(user_command, LOG_NAME, SEND_OR_RECEIVE);
...
void send_compressed_msg() {
  ...
  while (1) {
    ...
    char *user_command_compressed =
        compress_buffer(user_command, command_size, command_byte_size);

    // This part involves three sends, which are necessary to uncompress on the
    // server's side The first one sends the command original size, before
    // compression The second one sends the compressed command size The third
    // one sends the compressed command

    if (send(socket_connection, &command_size, sizeof(ulong), 0) == -1)
      error_output((char *)"Could Not Send Command Size");
    if (send(socket_connection, &command_byte_size, sizeof(ulong), 0) == -1)
      error_output((char *)"Could Not Send Compressed Command Size");
    if (send(socket_connection, user_command_compressed, command_byte_size,
             0) == -1)
      error_output((char *)"Could Not Send Compressed Command");

    // Change log flag to send
    SEND_OR_RECEIVE = 0;
    if (LOG_SAVE)
      update_log_compressed(user_command, user_command_compressed, LOG_NAME,
                            SEND_OR_RECEIVE);
...
void recv_msg() {
  ...
  while (1) {
    ...
    if (receive > 0) {
      ...
      SEND_OR_RECEIVE = 1;
      if (LOG_SAVE)
        update_log(output, LOG_NAME, SEND_OR_RECEIVE);

      printf("\033[33m%s\n", output);
    } else if (receive == 0)
      break;
    else
      error_output((char *)"Could Not Receive Output Without Compression");
...
void recv_compressed_msg() {
  while (1) {
    ...
    ulong output_size = 0, output_byte_size = 0;
    ...
    if (receive > 0) {
      if (recv(socket_connection, &output_byte_size, sizeof(ulong), 0) == -1)
      ...
      if (recv(socket_connection, tmp, output_byte_size, 0) == -1)
        error_output((char *)"Could Not Send Compressed Output");

      // Decompresses output using zlib's uncompress()
      output_uncompressed =
          uncompress_buffer(tmp, output_size, output_byte_size);

      // Change log flag to receive
      SEND_OR_RECEIVE = 1;
      if (LOG_SAVE)
        update_log_compressed(output_uncompressed, tmp, LOG_NAME,
                              SEND_OR_RECEIVE);
      ...
      error_output((char *)"Could Not Receive Output Size");
...
int main(int argc, char *argv[]) {
  ...
  test_if_options_passed_client(argc, argv, &LOG_NAME, &PORT, &HOST, &COMPRESS,
                                &LOG_SAVE);
  ...
  if (send(socket_connection, name, LEN, 0) == -1)
    error_output((char *)"Could Not Send Client's Name");
  ...
  if (compress_flag == -1)
    error_output((char *)"Could Not Send Compress Flag");

  printf("=== TERMINAL STARTED ===\n");

  // Two threads are initiated per client, one to send messages and another one
  // to receive messages
  pthread_t send_msg_thread, recv_msg_thread;

  // Checking if data compression is activated to choose the correct thread
  // handlers
  if (COMPRESS) {
    if (pthread_create(&send_msg_thread, NULL, (void *)send_compressed_msg,
                       NULL) != 0)
      error_output((char *)"Could Not Create Compressed Send Message Thread");
    if (pthread_create(&recv_msg_thread, NULL, (void *)recv_compressed_msg,
                       NULL) != 0)
      error_output(
          (char *)"Could Not Create Compressed Receive Message Thread");
  } else {
    if (pthread_create(&send_msg_thread, NULL, (void *)send_msg, NULL) != 0)
      error_output((char *)"Could Not Create Uncompressed Send Message Thread");
    if (pthread_create(&recv_msg_thread, NULL, (void *)recv_msg, NULL) != 0)
      error_output(
          (char *)"Could Not Create Uncompressed Receive Message Thread");
  }

  // Waiting for the client to press ^D and disconnect from server and destroy
  // mutex
  while (1) {
    if (disconnect) {
